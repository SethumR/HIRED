...
// Updated chart colors with the new color (#6be3a3)
const chartColors = [
  "#87552d", // Dark brown
  "#38736f", // Dark teal
  "#30638a", // Dark blue
  "#442f6e", // Dark purple
  "#193b29", // New color (light green)
];
...
const skillImprovement = [
  ...
  { skill: "Technical ", improvement: 75 },
...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-gray-900 border border-purple-500/20 rounded-lg p-2 text-sm"> {/* Reduced padding and text size */}
        <p className="text-purple-400  text-[15px]">{label}</p> {/* Reduced font size */}
        <p className="text-xs"> {/* Reduced font size */}
...
function TipsSection() {
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6 mt-16 mb-20"> {/* Added margin-top and margin-bottom */}
      {tips.map((item, index) => (
                ...
                <AreaChart data={overallGrowth}>
                  <defs>
                    <linearGradient id="colorScore" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#6be3a3" stopOpacity={0.8} /> {/* Updated color */}
                      <stop offset="95%" stopColor="#6be3a3" stopOpacity={0} /> {/* Updated color */}
                    </linearGradient>
                  </defs>
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: "#6be3a3" }} 
                    itemStyle={{ color: "#fff" }}
                  />
                  <Area
                    type="monotone"
                    dataKey="score"
                    stroke="#6be3a3" // Updated stroke color
                    fillOpacity={1}
                    fill="url(#colorScore)"
                    strokeWidth={2}
                    dot={{ fill: "#6be3a3", strokeWidth: 0 }} // Updated dot color
                    activeDot={{ r: 6, fill: "#6be3a3" }} // Updated active dot color
                  />
                </AreaChart>
              </ResponsiveContainer>
            </div>
